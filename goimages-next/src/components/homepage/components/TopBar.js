...
export default async function TopBar({ route }) {
    ...
    if (routes.some((r) => r.route === route)) {
        return <div className="flex gap-2 max-w-[1400px] border border-t-0 border-tirtiary opacity-60 rounded-b-md mx-auto px-3 py-1 animate-fade-in-down">
            {routes.map(x => <Link className={`w-fit text-center transition-all ease-in-out duration-200 rounded-md py-2 px-3 sm:order-2 order-1 flex justify-center items-center gap-1 ${x.route === route ? activeStyle : inactiveStyle}`} href={x.route} alt={x.content}>
